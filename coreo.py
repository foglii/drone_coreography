...
drone1_2=drone1_txt[len(drone1_txt)//2:].tolist()
...
drone2_2=drone2_txt[len(drone2_txt)//2:].tolist()
...
drone4_2=drone4_txt[len(drone4_txt)//2:].tolist()
...
drone5_2=drone5_txt[len(drone5_txt)//2:].tolist()
...
drone6_2=drone6_txt[len(drone6_txt)//2:].tolist()
...
def run_trajectory(scf, trajectory_id, duration):
    try:
        ...
        if iterr==2:
                ...
                time.sleep(3.0)
                commander.stop()
        else:  commander.go_to(0, 0, 0.1, 0, 1, relative=True, linear=True)
